...
export function getAllVideogames() {
    return async (dispatch) => {
        ...
        try {
            const response = await axios.get('https://pi-videogames-production-45a7.up.railway.app/videogames');
            dispatch({
...
export function getVideogameById(id){
    return async (dispatch) => {
        try{
            const infoAPI = (await axios.get(`https://pi-videogames-production-45a7.up.railway.app/videogames/${id}`)).data;
...
export function getVideogamesByName(name){
    return async (dispatch) => {
        try{
            dispatch(setIsLoading(true)); // Inicia el estado de carga
            const infoAPI = (await axios.get(`https://pi-videogames-production-45a7.up.railway.app/videogames/?name=${name}`)).data;
...
export function postVideogames(videogame){
    return async (dispatch) => {
        try {
            const infoAPI = (await axios.post(`https://pi-videogames-production-45a7.up.railway.app/videogames`, videogame)).data;
...
export function getAllGenres(){
    return async (dispatch) => {
        try {
            const infoAPI = (await axios.get(`https://pi-videogames-production-45a7.up.railway.app/genres`)).data;
